...
const NoDeposits = () => {
  ...
  const [noDepositUsers, setNoDepositUsers] = useState([]);
  ...
  const fetchUsersWithFewDeposits = async (countryId) => {
    if (!countryId) return;

    setLoadingUsers(true);
    try {
      const response = await systemApi.getUsersWithFewDeposits(globalState?.key, countryId, TOP_SECRET_KEY);

      if (response.data) {
        const { data: responseData, status } = response;
        if (status === 200 || status === 201) {
          const usersData = responseData.data || responseData;
          setNoDepositUsers(Array.isArray(usersData) ? usersData : []);
        }
      } else {
        setNoDepositUsers([]);
      }
    } catch (error) {
      console.log('Error fetching users with few deposits:', error);
  ...
  const fetchAllCountriesStats = async () => {
    ...
    try {
      ...
      for (const country of countries) {
        const response = await systemApi.getUsersWithFewDeposits(globalState?.key, country._id, TOP_SECRET_KEY);
  ...
  useEffect(() => {
    if (selectedCountry && globalState?.key) {
      fetchUsersWithFewDeposits(selectedCountry);
    }
  ...
  const filteredUsers = noDepositUsers.filter((user) => {
  ...
  const exportEmailsToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Utilisateurs sans activité');
      ...
      // Generate filename
      const filename = `utilisateurs_sans_activite_${countryName}_${currentDate}.xlsx`;
  ...
  if (isLoading) {
    return (
      <MainCard title="Utilisateurs sans activités">
        <Box>
                  ...
                  <TableCell colSpan={5} align="center">
                    <Box sx={{ py: 3 }}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {selectedCountry ? 'Aucun utilisateur sans activité trouvé' : 'Sélectionnez un pays pour voir les utilisateurs'}
                      </Typography>
                    </Box>
                  </TableCell>
                </TableRow>
